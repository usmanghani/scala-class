...
abstract class TweetSet {
   ...
   */
  def filter(p: Tweet => Boolean): TweetSet = this.filter0(p, null)
  ...
  def ascendingByRetweet: Trending = {
    ...
    def ascendingByRetweet0(tweets: TweetSet, accu: Trending): Trending = {
    	if (tweets.isEmpty) accu
    	else ascendingByRetweet0(tweets.remove(tweets.findMin), accu + tweets.findMin)
    }
